...
import kotlinx.android.synthetic.main.activity_on_chat.*
import kotlinx.android.synthetic.main.toolbar.*

class OnChatActivity : AppCompatActivity() {
    ...
    private var mTentorUid: String? = null
    ...
    fun initialize(){
        ...
        mTentorUid = intent.getStringExtra("tentor_uid")

        val layoutManager = LinearLayoutManager(this)
        layoutManager.stackFromEnd = true
        mAdapter = MessageAdapter(this, mMessageList, mUser?.uid)
        onchat_recyclerview.layoutManager = layoutManager
        onchat_recyclerview.adapter = mAdapter
    ...
    fun getTentor(){
        mTentorUid?.let { mRef.child("tentor").child(it).addListenerForSingleValueEvent(object : ValueEventListener{
    ...
    fun checkMessage(){
        val message = onchat_msg.text.toString()
        if (message.isBlank()){
            onchat_msg.error = "Pesan belum di isi"
        }else{
            sendMessage(message)
        }
    }
        ...
        tokenRef.child("token").orderByKey().equalTo(mTentor?.uid)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    var data = Message((mMessageList.size+1).toLong(),"Admin"
                        ,"eKAwdUASOkfuJsxqnUJyH3patcX2",mTentor?.name,mUser?.uid,"",message)
        ...
        mTentorUid?.let {
            mRef.child("tentor").child(it).child("message").child(
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    ...
                    //to me
                    mRef.child("admin").child("eKAwdUASOkfuJsxqnUJyH3patcX2").child("message").child(mTentor?.uid.toString())
                    ...
                    //to tentor
                    mRef.child("tentor").child(mTentor?.uid.toString()).child("message").child("eKAwdUASOkfuJsxqnUJyH3patcX2")
    ...
    fun getMessage(){
        mRef.child("admin").child("eKAwdUASOkfuJsxqnUJyH3patcX2").child("message")
    ...
    fun onBottomScrolling(){
        ...
        val layoutManager = onchat_recyclerview.layoutManager as LinearLayoutManager
